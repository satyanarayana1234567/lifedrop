    ...
    @if(request('blood_group') || request('city'))
        ...
        <div class="nearby-notice reveal">
            ...
            <div>
                <div style="font-weight:700;font-size:17px;color:#92400e;margin-bottom:6px;">
                    No donors found in <strong>{{ $searchedCity ?: 'your city' }}</strong>
                    @if($searchedGroup) for blood group <strong>{{ $searchedGroup }}</strong>@endif
                </div>
                <p style="color:#78350f;font-size:14px;line-height:1.7;margin:0;">
                    ...
                    <a href="{{ route('donors.create') }}" style="color:#c0152a;font-weight:700;">register yourself</a>
                    to help grow the donor network.
                </p>
                <div class="d-flex gap-2 mt-3 flex-wrap">
                    <a href="{{ route('donors.search') }}?blood_group={{ $searchedGroup }}"
        ...
        <div class="results-header reveal">
            <h4>
                ...
                Donors Found
                @if($searchedCity) in <em>{{ $searchedCity }}</em>@endif
            </h4>
            <div class="result-count-chip">🩸 {{ count($donors) }} {{ count($donors) == 1 ? 'donor' : 'donors' }} available</div>
        </div>

        <div class="row g-4">
            @foreach($donors as $donor)
            <div class="col-lg-4 col-md-6 reveal reveal-delay-{{ ($loop->index % 3) + 1 }}">
                <div class="donor-card">

                    <!-- Top: photo + name + blood -->
                    <div class="donor-top">
                        @if($donor->photo)
                            <img src="{{ asset($donor->photo) }}" class="donor-photo" alt="{{ $donor->name }}">
                        @else
                    ...
                    <div class="donor-info-row" style="border-bottom:none;">
                        <i class="fas fa-circle" style="font-size:8px;animation:blink 1.5s infinite;color:{{ $donor->availability=='Available' ? '#16a34a' : '#9ca3af' }};"></i>
                    ...
                    <!-- WhatsApp -->
                    <a href="https://wa.me/91{{ $donor->phone }}?text=Hello%20{{ urlencode($donor->name) }},%20I%20found%20your%20contact%20on%20LifeDrop.%20We%20need%20blood%20group%20{{ urlencode($donor->blood_group) }}.%20Are%20you%20available%20to%20donate?"
